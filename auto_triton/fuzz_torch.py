...
OP_REGISTRY: List[Callable] = [
    op_relu, op_add_scalar, op_mul_scalar,
    op_clone, op_flatten, op_transpose, op_matmul,
    op_sigmoid, op_tanh, op_softmax, op_einsum
]
...
def generate_compositional_kernel(
    *,
    ...
    lines.append(f"t0 = torch.randn(({X}, {M}, {K}), dtype=torch.float32")
    env["t0"] = t0

    # t1 : (N, M, K)
    t1 = TensorType((N, M, K), "float32")
    lines.append(f"t1 = torch.randn(({N}, {M}, {K}), dtype=torch.float32)")
    env["t1"] = t1

    # t2 : (K, L)
    t2 = TensorType((K, L), "float32")
    lines.append(f"t2 = torch.randn(({K}, {L}), dtype=torch.float32)")
    env["t2"] = t2
...
if __name__ == "__main__":
    ...
    for i in range(999):
        print(f"\n# ── Kernel {i+1} ──")
        xxx = generate_compositional_kernel(depth=10)
        fuzzed_code.add(xxx)
        print(xxx)
